...
def download_news_video_and_content(
    ...
    audio_raw_path = os.path.join(audio_dir + "/../audio_raw/", "{}_raw.wav".format(news_id))
    # prof_path = os.path.join(audio_dir + "/../audio_raw/", "{}.prof".format(news_id))
    ...
    try:
        article_title = soup.find_all('article_title')
        print(news_id)
        print(article_title)
        ...
        if not os.path.exists(audio_path):
            ...
            # # prof 파일 생성
            # command = 'sox {} -n noiseprof {}  '.\
            #     format(audio_raw_path, prof_path)
            # subprocess.call(command, shell=True)

            # # 잡음 제거
            # command = 'sox -S --multi-threaded --buffer 131072 {} {} noisered {} 0.21 norm;'.\
            #     format(audio_raw_path, audio_path , prof_path)
            # subprocess.call(command, shell=True)
