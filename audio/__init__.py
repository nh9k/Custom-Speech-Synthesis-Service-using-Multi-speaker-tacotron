...
def combine_audio(INPUT_PATH):
    # 경로에 파일이 없으면 실행 안함
    if os.path.exists(INPUT_PATH):
        FILE_DIR_NAMES = os.listdir(INPUT_PATH)     # audio 폴더안에 있는 파일/폴더 이름 불러오기
        WAVE_NAMES = []                             # .wav 파일만 남기기
        waves = []                                  # 불러온 .wav 파일들
        
        # 파일 리스트 중 .wav 파일만 남기고 제거
        for paths in FILE_DIR_NAMES:
            if paths.find('.wav') != -1:
                WAVE_NAMES.append(paths)

        # .wav 파일 불러오기
        for paths in WAVE_NAMES:
                waves.append(librosa.load(os.path.join(INPUT_PATH, paths)))
        ...
        # 파일 출력
        OUTPUT_WAVE_PATH = os.path.join(INPUT_PATH, 'output.wav')
...
#만약 app.py 실행시 웹에서 소리가 안나온다면 이걸로 실행: librosa 0.5.1
...
#학습할 때 500step에서 끊기면 librosa 문제로, 함수 모양이 바뀌어서 그럼. 이 부분을 대체해서 쓰면 됨
#librosa 0.6.2, 0.6.3
#def save_audio(audio, path, sample_rate=None):
#    #audio *= 32767 / max(0.01, np.max(np.abs(audio)))
#    librosa.output.write_wav(path, audio, #.astype(np.int16)
