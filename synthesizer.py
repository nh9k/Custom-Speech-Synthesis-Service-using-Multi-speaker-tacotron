...
class Synthesizer(object):
    ...
    def load(self, checkpoint_path, num_speakers=2, checkpoint_step=None, model_name='tacotron'):
        ...
        #tf.reset_default_graph()#악몰라ㅜ어떡해 무섭지만 해봐야지 11월 9일 오후 1시
...
if __name__ == "__main__":
    ...
    audio = synthesizer.synthesize(
            texts=[config.text],
            base_path=config.sample_path, #base_path=config.load_path
            speaker_ids=[config.speaker_id],
            attention_trim=True, #original False
